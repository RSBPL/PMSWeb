...
{
    public class BulkstorageLocationController : Controller
    {
        // GET: BulkstorageLocation
        OracleCommand cmd;
        ...
        public JsonResult getPlanVertical(string PLANTCODE)
        {
            ...
            var gridright = new List<BULKRIGHTGRID>();
            string plant = "", DisplayMethod = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(data))
                {
                    ...
                    {
                        query = String.Format(@"SELECT A.* ,TO_NUMBER(NVL(CAPACITY, 0)) - A.QTY AVAILABLE FROM(SELECT B.ITEM_CODE, R.ITEM_DESCRIPTION, B.LOCATION_CODE, B.CAPACITY, SAFTY_STOCK_QUANTITY, SUM(I.QUANTITY) QTY, (FN_GETSTORGE_LOCATION('TEMP', B.PLANT_CODE, B.FAMILY_CODE, B.ITEM_CODE)) TEMP_LOCATION FROM XXES_BULK_STORAGE B JOIN XXES_BULKSTOCK I ON B.PLANT_CODE = I.PLANT_CODE AND B.FAMILY_CODE = I.FAMILY_CODE AND B.ITEM_CODE = I.ITEMCODE AND B.LOCATION_CODE = I.LOCATION_CODE JOIN XXES_RAWMATERIAL_MASTER R ON B.PLANT_CODE = R.PLANT_CODE AND B.FAMILY_CODE = R.FAMILY_CODE AND B.ITEM_CODE = R.ITEM_CODE WHERE NVL(B.TEMP_LOC, '') <> 'Y' AND B.PLANT_CODE = '{0}' AND TO_NUMBER(NVL(B.SAFTY_STOCK_QUANTITY, 0)) > NVL(TO_NUMBER((SELECT SUM(TO_NUMBER(I.QUANTITY)) FROM XXES_BULKSTOCK K WHERE K.PLANT_CODE = B.PLANT_CODE AND K.FAMILY_CODE = B.FAMILY_CODE AND K.ITEMCODE = B.ITEM_CODE AND K.LOCATION_CODE = B.LOCATION_CODE)), 0) GROUP BY B.ITEM_CODE, R.ITEM_DESCRIPTION, B.LOCATION_CODE, B.CAPACITY, SAFTY_STOCK_QUANTITY, B.PLANT_CODE, B.FAMILY_CODE) A", plant.Trim());
                    ...
                    else 
                    {
                        try
                        {
                            using (OracleCommand sc = new OracleCommand("USP_GETLIVEDATA", fun.Connection()))
                            {
                                ...
                                sc.Parameters.Add("PROCESS", OracleDbType.NVarchar2, ParameterDirection.Input).Value = "BSL";
                    ...
                    try
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow dr in dt.Rows)
                            {
                                ...
                                TblMdl.ITEM_CODE = dr["ITEM_CODE"].ToString();
                                TblMdl.ITEM_DESCRIPTION = dr["ITEM_DESCRIPTION"].ToString();
                                TblMdl.LOCATION_CODE = dr["LOCATION_CODE"].ToString();
                                TblMdl.SAFTY_STOCK_QUANTITY = dr["SAFTY_STOCK_QUANTITY"].ToString();
                                TblMdl.CAPACITY = dr["CAPACITY"].ToString();
                                TblMdl.QTY = dr["QTY"].ToString();
                                TblMdl.AVAILABLE = dr["AVAILABLE"].ToString();
                                TblMdl.TEMP_LOCATION = Convert.ToString(dr["TEMP_LOCATION"]);
                                if (!string.IsNullOrEmpty(TblMdl.TEMP_LOCATION))
                                {
                                    query = string.Format(@"SELECT nvl(SUM(K.QUANTITY),0) FROM XXES_BULKSTOCK K JOIN XXES_BULK_STORAGE S ON K.ITEMCODE=S.ITEM_CODE
                                AND K.PLANT_CODE=S.PLANT_CODE AND K.FAMILY_CODE=S.FAMILY_CODE AND K.LOCATION_CODE='{1}'
                                and k.plant_code='{0}'
                                GROUP BY K.PLANT_CODE,K.FAMILY_CODE,K.LOCATION_CODE", plant, TblMdl.TEMP_LOCATION);

                                    TblMdl.TEMP_QTY = fun.get_Col_Value(query);
                                }
                                else { TblMdl.TEMP_QTY = "0"; }
        ...
        public class BULKRIGHTGRID
        {
            public string CAPACITY { get; set; }
            public string LOCATION_CODE { get; set; }
            public string ITEM_DESCRIPTION { get; set; }
            public string ITEM_CODE { get; set; }
            public string SAFTY_STOCK_QUANTITY { get; set; }
        ...
        public class TblColomnData
        {
            ...
            public string TEMP_LOCATION { get; set; }
            public string TEMP_QTY { get; set; }
        ...
        public class TblColomn
        {
            ...
            public virtual ICollection<BULKRIGHTGRID> gridright { get; set; }
        }
    }
}