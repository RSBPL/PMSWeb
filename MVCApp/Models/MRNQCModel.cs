	...
	public class AddMRN
    {
		...
		public List<MRNQCVIEWMODEL> GridMRN(MRNQCVIEWMODEL obj)
		{
			...
			try
			{
				...
				query = string.Format(@"SELECT TOTALCOUNT,TRANSACTION_DATE,VENDOR_NAME,VENDOR_CODE,MRN_NO,DESCRIPTION,ITEMCODE,
						ITEM_REVISION,QUANTITY,	PUNAME,COMMODITY,BUYER,INSPECTOR,SKIPLOT_RATIO,
								ISDIM,ISMT,
								DIMOK_QTY1,DIMREJ_QTY1,DIMOK_DEV1,        
								DIMOK_AFTERSEG1,   
								DIMOK_AFTEREWORK1,
								DIMHOLD_QTY1,
								DIM_DATE1, 
								...
								NUMTIMES_REC,
								STORAGE_LOC, 
								CASE WHEN SHEET_AVAILNOT < 1 THEN 'Not Available' ELSE 'Available' END AS SHEET_AVAILNOT,
								DEVIATION_BALQTY,PARTHANDOVER_STATUS,
								CASE WHEN	PPM_3MONTH = -99 THEN 'N/A' ELSE TO_CHAR(PPM_3MONTH) END AS PPM_3MONTH,
								PPM_3DATE,
								CASE WHEN PPM_2MONTH = -99 THEN 'N/A' ELSE TO_CHAR(PPM_2MONTH) END AS PPM_2MONTH,
								PPM_2DATE,
								CASE WHEN PPM_1MONTH = -99 THEN 'N/A' ELSE TO_CHAR(PPM_1MONTH) END AS	PPM_1MONTH,
								PPM_1DATE,
								AVG3MONTH_PPM,
								MRNAGEING_STATUS,DOCK_QTY,PAINTSHOP_QTY,
								LINE_QTY,STORELOC_QTY,
								PO_REV,
								BOM_REVISION,PLANT_CODE,DIM_STATUS,MT_STATUS,FITMENT_STATUS,
								SQA_STATUS
 
								FROM (SELECT ROW_NUMBER() OVER (ORDER BY ird.TRANSACTION_DATE) as ROWNMBER,
								COUNT(*) over() as TOTALCOUNT, to_char(IRD.TRANSACTION_DATE, 'DD-Mon-YYYY HH24:MI:SS') as TRANSACTION_DATE,IRD.VENDOR_NAME,IRD.VENDOR_CODE,
								MRI.MRN_NO,SUBSTR(mri.ITEM_DESCRIPTION,1,35) AS DESCRIPTION,
								MRI.ITEMCODE,MRI.ITEM_REVISION,MRI.QUANTITY,MRI.PUNAME,
								(SELECT CATEGORY2 || '-' || CATEGORY3 FROM APPS.XXES_ONHAND_ITEM_V 
								WHERE ORGANIZATION_ID = '{6}' AND ITEM_CODE = mri.ITEMCODE AND ROWNUM = 1) AS COMMODITY,
								(SELECT BUYER_NAME FROM APPS.XXES_ONHAND_ITEM_V WHERE ORGANIZATION_ID = '{6}' 
								AND ITEM_CODE = mri.ITEMCODE AND ROWNUM = 1) AS BUYER,
								MRNQC.INSPECTOR,
								slp.FREQUENCY_NUM || ':' ||slp.FREQUENCY_DENOM AS SKIPLOT_RATIO,
								slp.PLAN_TYPE AS ISDIM, 
								slp.PLAN_TYPE AS ISMT,
								MHS.DIMOK_QTY1,       
								MHS.DIMREJ_QTY1,       
								MHS.DIMOK_DEV1,        
								MHS.DIMOK_AFTERSEG1,   
								MHS.DIMOK_AFTEREWORK1,
								MHS.DIMHOLD_QTY1,
								MHS.DIM_DATE1,
								MHS.DIM_INSPECTIONDATE1,
								MHS.DIMOK_QTY2,        
								MHS.DIMREJ_QTY2,       
								MHS.DIMOK_DEV2,        
								MHS.DIMOK_AFTERSEG2,   
								MHS.DIMOK_AFTEREWORK2, 
								MHS.DIMHOLD_QTY2,
								MHS.DIM_DATE2,
								MHS.DIM_INSPECTIONDATE2,
								MHS.DIMOK_QTY3,        
								MHS.DIMREJ_QTY3,       
								MHS.DIMOK_DEV3,        
								MHS.DIMOK_AFTERSEG3,   
								MHS.DIMOK_AFTEREWORK3,
								MHS.DIMHOLD_QTY3,
								MHS.DIM_DATE3,
								MHS.DIM_INSPECTIONDATE3,
								MHS.MTOK_QTY1,
								MHS.MTREJ_QTY1,
								MHS.MTOK_DEV1,
								MHS.MTOK_AFTERSEG1,
								MHS.MTOK_AFTEREWORK1,
								MHS.MTHOLD_QTY1,
								MHS.MT_DATE1,
								MHS.MT_INSPECTIONDATE1,
								MHS.MTOK_QTY2,
								MHS.MTREJ_QTY2,
								MHS.MTOK_DEV2,
								MHS.MTOK_AFTERSEG2,
								MHS.MTOK_AFTEREWORK2,
								MHS.MTHOLD_QTY2,
								MHS.MT_DATE2,
								MHS.MT_INSPECTIONDATE2,
								MHS.MTOK_QTY3,
								MHS.MTREJ_QTY3,
								MHS.MTOK_DEV3,
								MHS.MTOK_AFTERSEG3,
								MHS.MTOK_AFTEREWORK3,
								MHS.MTHOLD_QTY3,
								MHS.MT_DATE3,
								MHS.MT_INSPECTIONDATE3,
								xgp.po_release_status AS BLANKET_STDPO, 

								'' AS NEW_REGULAR,
								(SELECT COUNT(*) FROM XXES_MRNINFO MRINF
								INNER JOIN ITEM_RECEIPT_DETIALS irdS
								ON MRINF.MRN_NO = irdS.MRN_NO AND MRINF.PLANT_CODE = irdS.PLANT_CODE AND MRINF.FAMILY_CODE = irdS.FAMILY_CODE
								WHERE TO_DATE(irdS.TRANSACTION_DATE,'dd-Mon-yyyy HH24:MI:SS') < TO_DATE(IRD.TRANSACTION_DATE,'dd-Mon-yyyy HH24:MI:SS') AND irdS.VENDOR_CODE = IRD.VENDOR_CODE  AND MRINF.ITEMCODE = MRI.ITEMCODE
								AND MRINF.PLANT_CODE = MRI.PLANT_CODE AND MRINF.FAMILY_CODE = MRI.FAMILY_CODE) NUMTIMES_REC,
								qrv.character4 AS STORAGE_LOC, 
								
								(SELECT COUNT(*) FROM APPS.QA_PLANS WHERE NAME LIKE '%'||mri.ITEMCODE||'%' AND ORGANIZATION_ID = '{6}'
								) SHEET_AVAILNOT,

								FN_DEVIATON(IRD.VENDOR_CODE,MRI.ITEMCODE,MRI.PLANT_CODE,MRI.FAMILY_CODE) AS DEVIATION_BALQTY,
								MRNQC.PARTHANDOVER_STATUS,
								FN_PPM(IRD.VENDOR_CODE,MRI.ITEMCODE,TRUNC(IRD.TRANSACTION_DATE),3,MRI.PLANT_CODE,MRI.FAMILY_CODE) AS PPM_3MONTH,
								TO_CHAR(add_months(trunc(IRD.TRANSACTION_DATE,'mm'),-3),'MON-YYYY') AS PPM_3DATE,
								FN_PPM(IRD.VENDOR_CODE,MRI.ITEMCODE,TRUNC(IRD.TRANSACTION_DATE),2,MRI.PLANT_CODE,MRI.FAMILY_CODE) AS PPM_2MONTH,
								TO_CHAR(add_months(trunc(IRD.TRANSACTION_DATE,'mm'),-2),'MON-YYYY') AS PPM_2DATE, 
								FN_PPM(IRD.VENDOR_CODE,MRI.ITEMCODE,TRUNC(IRD.TRANSACTION_DATE),1,MRI.PLANT_CODE,MRI.FAMILY_CODE) AS PPM_1MONTH,
								TO_CHAR(add_months(trunc(IRD.TRANSACTION_DATE,'mm'),-1),'MON-YYYY') AS PPM_1DATE, 
								CASE WHEN  FN_PPM3MONTH(IRD.VENDOR_CODE,MRI.ITEMCODE, TRUNC(IRD.TRANSACTION_DATE),MRI.PLANT_CODE,MRI.FAMILY_CODE) < 0 THEN slp.FREQUENCY_NUM || ':' ||slp.FREQUENCY_DENOM  
								   WHEN FN_PPM3MONTH(IRD.VENDOR_CODE,MRI.ITEMCODE, TRUNC(IRD.TRANSACTION_DATE),MRI.PLANT_CODE,MRI.FAMILY_CODE) = 0 THEN '1:50'
								   WHEN  FN_PPM3MONTH(IRD.VENDOR_CODE,MRI.ITEMCODE, TRUNC(IRD.TRANSACTION_DATE),MRI.PLANT_CODE,MRI.FAMILY_CODE) >= 1 AND FN_PPM3MONTH(IRD.VENDOR_CODE,MRI.ITEMCODE, TRUNC(IRD.TRANSACTION_DATE),MRI.PLANT_CODE,MRI.FAMILY_CODE) <= 1999 THEN '1:10'
								   WHEN  FN_PPM3MONTH(IRD.VENDOR_CODE,MRI.ITEMCODE, TRUNC(IRD.TRANSACTION_DATE),MRI.PLANT_CODE,MRI.FAMILY_CODE) >= 2000 AND FN_PPM3MONTH(IRD.VENDOR_CODE,MRI.ITEMCODE, TRUNC(IRD.TRANSACTION_DATE),MRI.PLANT_CODE,MRI.FAMILY_CODE) <= 9999 THEN '1:5'
								  ELSE
								  '1:1'
								END AS AVG3MONTH_PPM,
								MRNQC.MRNAGEING_STATUS,
								
								(SELECT CASE WHEN SUM(ONHAND) IS NULL THEN 0 ELSE SUM(ONHAND) END FROM APPS.XXES_ONHAND_ITEM_V WHERE ORGANIZATION_ID = '{6}' 
								AND SUBINVENTORY_CODE LIKE '%RTV%' AND ITEM_CODE = MRI.ITEMCODE) AS DOCK_QTY,  
								(SELECT CASE WHEN SUM(ONHAND) IS NULL THEN 0 ELSE SUM(ONHAND) END FROM APPS.XXES_ONHAND_ITEM_V WHERE ORGANIZATION_ID = '{6}' 
								AND SUBINVENTORY_CODE LIKE '%RTV%' AND ITEM_CODE = MRI.ITEMCODE) AS PAINTSHOP_QTY,
												(SELECT CASE WHEN SUM(ONHAND) IS NULL THEN 0 ELSE SUM(ONHAND) END FROM APPS.XXES_ONHAND_ITEM_V WHERE ORGANIZATION_ID = '{6}' 
								AND SUBINVENTORY_CODE LIKE '%RTV%' AND ITEM_CODE = MRI.ITEMCODE) AS LINE_QTY,
								(SELECT CASE WHEN SUM(ONHAND) IS NULL THEN 0 ELSE SUM(ONHAND) END FROM APPS.XXES_ONHAND_ITEM_V WHERE ORGANIZATION_ID = '{6}' 
								AND SUBINVENTORY_CODE LIKE '%RTV%' AND ITEM_CODE = MRI.ITEMCODE) AS STORELOC_QTY,
								
								MRI.ITEM_REVISION AS PO_REV,
								MRI.BOM_REVISION,MRI.PLANT_CODE,
								FUN_ISIR('{6}','187125') AS DIM_STATUS,
								FUN_ISIR('{6}','187125') AS MT_STATUS,
								FUN_ISIR('{6}','187125') AS FITMENT_STATUS,
								SQASTATUS('{6}',MRI.ITEMCODE,MRI.ITEM_REVISION) AS SQA_STATUS
								FROM XXES_MRNINFO mri 
								INNER JOIN ITEM_RECEIPT_DETIALS ird
								ON MRI.MRN_NO = IRD.MRN_NO AND MRI.PLANT_CODE = IRD.PLANT_CODE AND MRI.FAMILY_CODE = IRD.FAMILY_CODE

								LEFT OUTER JOIN XXES_MRNQC mrnqc
								ON mrnqc.PLANT_CODE = MRI.PLANT_CODE AND mrnqc.FAMILY_CODE = MRI.FAMILY_CODE AND mrnqc.ITEMCODE = MRI.ITEMCODE 
								  AND mrnqc.VENDOR_CODE = IRD.VENDOR_CODE AND MRNQC.MRN_NO = MRI.MRN_NO
								LEFT OUTER JOIN  
								XXES_MRNQC_HISTORY mhs
								ON mhs.PLANT_CODE = MRI.PLANT_CODE AND mhs.FAMILY_CODE = MRI.FAMILY_CODE AND mhs.ITEMCODE = MRI.ITEMCODE 
								  AND mhs.VENDOR_CODE = IRD.VENDOR_CODE
								
								LEFT OUTER JOIN
								APPS.XXES_SKIP_LOT_PROCESS_V slp
								ON slp.ORG_CODE = mri.PLANT_CODE AND slp.VENDOR_CODE = ird.VENDOR_CODE AND slp.ITEM = mri.ITEMCODE 
              
								LEFT OUTER JOIN
								APPS.XXESGATETAGPRINT_BARCODE xgp
								ON xgp.ORGANIZATION_CODE = mri.PLANT_CODE AND xgp.VENDOR_CODE = ird.VENDOR_CODE AND xgp.MRN_NO = mri.MRN_NO AND xgp.ITEM_CODE = mri.ITEMCODE

								LEFT OUTER JOIN
								APPS.QA_RESULTS_V qrv 
								ON qrv.ORGANIZATION_ID = '{6}' AND qrv.VENDOR_ID = ird.VENDOR_CODE 
								WHERE 
								MRI.MRN_NO NOT IN(SELECT qc.MRN_NO FROM XXES_MRNQC qc WHERE qc.PLANT_CODE = MRI.PLANT_CODE AND QC.FAMILY_CODE = mri.FAMILY_CODE)
								AND mri.STORE_VERIFIED IS NOT NULL
								AND MRI.PLANT_CODE = '{0}' 
								AND MRI.FAMILY_CODE = '{1}' 
								AND TO_CHAR(ird.TRANSACTION_DATE,'dd-Mon-yyyy') >= TO_DATE('{2}','dd-Mon-yyyy')
								AND TO_CHAR(ird.TRANSACTION_DATE,'dd-Mon-yyyy') <= TO_DATE('{3}','dd-Mon-yyyy')) KB
								WHERE ROWNMBER > '{4}' and ROWNMBER <= '{5}'", obj.PLANT_CODE, obj.FAMILY_CODE, obj.FROMDATE, obj.TODATE, Convert.ToInt32(obj.start), obj.length, orgid);
